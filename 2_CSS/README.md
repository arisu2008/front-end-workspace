...
- !important > 인라인 방식 > 아이디 선택자 > 클래스 선택자 > 전체 선택자
...
rgba(red, green, blue, alpha(0~1)) | [(0, 0, 0) ~ (255, 255, 255)];
```
...
- 색상(hue)은 색상환으로 표시 (0~360, 360-RED, 120-GREEN, 240-BLUE)
- 채도(saturation)는 %로 표시 (0%는 회색톤, 100%는 순색)
- 밝기(brightness)는 %로 표시 (0%는 가정 어둡고, 100%는 가장 밝다)
...
- flex 아이템의 기본 크기 설정 (flex-direction이 row일 떄는 너비, column일 떄는 높이)
...
.item {
  flex: 1 1 auto;
  └> /* flex-gorw: 1; flex-shrink: 1; flex-basis: auto; */

 flex: 1 500px;
 └> /* flex-gorw: 1; flex-shrink: 1; flex-basis: 500px; */

 flex: 1;
 └> /* flex-gorw: 1; flex-shrink: 1; flex-basis: 0%; */
}
```
